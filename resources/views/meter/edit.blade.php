...
@section('content')
    ...
    <div class="container">
        <div class="row mt-5">
            <div class="col"></div>
            <div class="col-6">
                <form action="{{ route('meter.update', $meter->id) }} "method="post" id="newForm">
                    @csrf
                    <fieldset>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" inputmode="numeric" pattern="(^S?)\d+$" id="identifier" class="form-control" 
                                    name="serial_number" placeholder="Unique Meter Identifier" value="{{ $meter->mpxn }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="types" class="form-select" name="meter_type">
                                ...
                                @foreach ($types as $type)
                                    <option value="{{ $type->id }}" @if ($type->id == $meter->meter_type_id) selected @endif>{{ $type->name }} ({{ $type->description }})</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" id="eac" class="form-control" value="{{ $meter->estimated_annual_consumption }}"
                                    name="eac"  min=2000 max=8000 required>
                        </div>

                        <div class="mb-3">
                            <label for="date_installed" class="form-label">Date Installed</label>
                            <input type="date" id="date_installed" class="form-control" name="date_installed" 
    ...
    <script>
        $(document).ready(function() {
            $('#newForm').submit(function(e) {
                ...
                $.post("{{ route('meter.update', $meter->id) }}",  form.serialize(), function(data) {
                    window.location.href = "{{ route('meter.show', $meter->id) }}"
                }).fail(function(data) {
                    alert(data.error);
                });
